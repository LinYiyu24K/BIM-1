...
class ViewerConfigurator extends BaseComponent {
  ...
  //component mount上去之后，开始传递一些模型的信息，用来渲染模型
  ...
  //joe:这个函数似乎是连接view.Extension.ExtensionManagerToolbar的唯一通道，所以只能改这里
  loadDynamicExtension (viewer, extension, options) {
    ...
    return new Promise ((resolve, reject) => {
      ...
      if(options.isPop){
        ...
        console.log("Viewer.Configurator正在poping")//joe

        this.state.dbModel.layout.rightFlex = 0.3//joe  这个设置不知道是什么意思，但效果比较流畅
        this.runAnimation(0.7, 1.0, 1.0)//joe
      }else{

        this.state.dbModel.layout.rightFlex = 0.3//joe
        this.runAnimation(1.0, 0.7, 1.0)//joe
  ...
  pushRenderExtension (extension) {
    ...
    return new Promise (async(resolve) => {
      ...
      if(this.first == true){//joe
        this.first = false
        layout.rightFlex=0.001  //如果是第一次的话，就隐藏那个extension的component
      }else{
        layout.rightFlex=0.3
      }      
  ...
  //这个函数在extensionManager里面没有调用到，所以不用改
  @autobind
  popRenderExtension () {
